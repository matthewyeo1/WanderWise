...
class MapItineraryPageState extends State<MapItineraryPage> {
  ...
  void _navigateToInviteToCollabPage(String itineraryId) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => InviteToCollabPage(userId: userId, itineraryId: itineraryId)),
    );
  }
...
class MapItineraryPageState extends State<MapItineraryPage> {
  ...
  Future<void> _initializeUserId() async {
    ...
    if (user != null) {
      ...
      _itineraryStream = _itineraryService.streamItineraryItems(userId);
      _loadItineraryItems();
    } else {
      Navigator.pushReplacementNamed(context, '/login');
    }
  ...
  try {
    ...
    // Check if result is not null (optional)
  ...
  void _navigateToInviteToCollabPage(String itineraryId) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => InviteToCollabPage(
          userId: userId,
    ...
    final result = await Navigator.push<Map<String, dynamic>>(
      ...
      MaterialPageRoute(
        builder: (context) => EditItineraryPage(
          ...
          initialItem: _itineraryItems[index],
          onSave: (updatedItem) async {
            await _itineraryService.updateItinerary(userId, updatedItem['id'], updatedItem);
            await _loadItineraryItems();
          },
        ),
      ),
